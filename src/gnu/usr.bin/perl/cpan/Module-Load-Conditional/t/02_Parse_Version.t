...
sub _succeed {
    return grep { /\S/ } map { s/^\s*//; $_ } split "\n", q[
        ...
        *VERSION = \'1.01';
        ...
        ($GD::Graph::area::VERSION) = '$Revision$' =~ /\s([\d.]+)/;
        ($GD::Graph::axestype::VERSION) = '$Revision$' =~ /\s([\d.]+)/;
        ($GD::Graph::colour::VERSION) = '$Revision$' =~ /\s([\d.]+)/;
        ($GD::Graph::pie::VERSION) = '$Revision$' =~ /\s([\d.]+)/;
        ($GD::Text::Align::VERSION) = '$Revision$' =~ /\s([\d.]+)/;
        $VERSION = qv('0.0.1');
        ...
        ( $VERSION = q($Id$) ) =~ s/^.*\s+(\d+)\/(\d+)\/(\d+).*$/$1$2$3/; # all one line for MakeMaker
