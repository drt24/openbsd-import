...
my %versions = (q[$VERSION = '1.00']            => '1.00',
                q[*VERSION = \'1.01']           => '1.01',
                q[($VERSION) = q$Revision$ =~ /(\d+)/g;] => 32208,
                ...
                '$VERSION = sprintf("%d.%03d", q$Revision$ =~ /(\d+)\.(\d+)/);' => '3.074',
                '$VERSION = substr(q$Revision$, 10) + 2 . "";'                   => '4.8',
                q[our $VERSION = do { my @r = ( q$Revision$ =~ /\d+/g ); sprintf "%d." . "%02d" x $#r, @r };] => '2.07', # Fucking seriously?
